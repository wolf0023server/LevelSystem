...
tellraw @s [{"text":"§2Rate: "},{"score":{"name":"@e[tag=levelsystem,limit=1]","objective":"ls_rate"},"color": "green","clickEvent":{"action":"suggest_command","value":"/scoreboard players set @e[tag=levelsystem] ls_rate "},"hoverEvent":{"action":"show_text","contents":["§bクリックで値を変更"]}}]
